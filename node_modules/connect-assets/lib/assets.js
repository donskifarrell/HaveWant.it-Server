    ...
    ConnectAssets.prototype.createHelpers = function() {
      ...
      context.js = function(route, routeOptions) {
        ...
        if ((routeOptions != null)) {       
          if (routeOptions.datamain != null) {
             loadingKeyword = "data-main='" + routeOptions.datamain + "' ";
          }
          if (routeOptions.loadAll != null) {
              var assetJS, jsFilesRoot, loadJSDirectory, loadJSFileOrDirectory;
              jsFilesRoot = _this.options.src + "/js";
              assetJS = _this.options.helperContext.js;
              loadJSFileOrDirectory = function(path) {
                var assetName, stat;
                stat = fs.statSync(path);
                if (stat != null ? stat.isDirectory() : void 0) {
                  return loadJSDirectory(path);
                } else {
                  fileType = path.split(/[. ]+/).pop();
                  assetName = (((path.replace(jsFilesRoot, "")).replace(".coffee", "")).replace(".js", "").replace(".eco", "").replace(".jade", "")).slice(1);
              ...
              loadJSDirectory = function(dirPath) {
                var path, paths, _i, _len;
                paths = fs.readdirSync(dirPath);
                for (_i = 0, _len = paths.length; _i < _len; _i++) {
                path = paths[_i];
                loadJSFileOrDirectory("" + dirPath + "/" + path);
                }
                return true;
              };

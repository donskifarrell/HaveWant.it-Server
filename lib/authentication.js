...
module.exports = function Server(envConf) {
	everyauth.debug = envConf.debug;
	...
	// Facebook
	if (envConf.external && envConf.external.facebook) {
		everyauth.facebook
		.appId(envConf.external.facebook.appId)
		.appSecret(envConf.external.facebook.appSecret)
		.findOrCreateUser(function (session, accessToken, accessTokenExtra, facebookUserMetaData) {return true;})
		.redirectPath('/');
	}

	// Twitter
	if (envConf.external && envConf.external.twitter) {
		everyauth.twitter
		.myHostname(envConf.uri)
		.consumerKey(envConf.external.twitter.consumerKey)
		.consumerSecret(envConf.external.twitter.consumerSecret)
		.findOrCreateUser(function (session, accessToken, accessSecret, twitterUser) {return true;})
		.redirectPath('/');
	}

	// Github
	if (envConf.external && envConf.external.github) {
		everyauth.github
		.myHostname(envConf.uri)
		.appId(envConf.external.github.appId)
		.appSecret(envConf.external.github.appSecret)

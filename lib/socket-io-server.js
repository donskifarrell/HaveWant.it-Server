module.exports = function Server(expressInstance, sessionStore) {
	...
	io.sockets.on('connection', function(client) {
		var user = client.handshake.session.user ?
			client.handshake.session.user.name :
			'UID: ' + (client.handshake.session.uid || 'has no UID');
		...
		client.on('message', function(msg) {
			...
			io.sockets.in(client.handshake.sid)
				.send('socket.io relay message "' + msg +
					'" from: ' + user +
					' @ ' + new Date().toString().match(/[0-9]+:[0-9]+:[0-9]+/));
		});
